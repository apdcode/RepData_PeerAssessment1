...
The previous table shows the sum of the steps that have been replaced where dates have missing values for the number of steps.
The fact that the sum of 10641 is equal for all days reveals an interesting detail about the dataset. It turns out that all dates with missing values, has missing values for all intervals, and that there is no missing interval for all dates contained in the dataset.
This means that it would not matter much if the imputing process replaced missing values for each and every interval, or for each date.
