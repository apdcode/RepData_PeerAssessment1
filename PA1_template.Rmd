...
# Make new variable for dt_weekdaypattern in dt_weekdaypattern, indicating interval with max value
...
dt_weekdaypattern <- data.table(dt_weekdaypattern, stepmax)

#________Calculations on weekends
# Average steps per interval for each weekend
dt_weekendpattern <- data.table(sqldf("SELECT interval, avg(steps) as steps, endofweek 
                            FROM dt_imputed2
                            WHERE endofweek = 'weekend'
                            Group by interval"))
...
# Make new variable for dt_weekendpattern in dt_weekendpattern, indicating interval with max value
stepmax <- 1:nrow(dt_weekendpattern)

for(i in 1:nrow(dt_weekendpattern)) {
    if (is.na(dt_weekendpattern$steps[i]) == TRUE) {
        stepmax[i] <- NA
    }else{
        if (is.na(dt_weekendpattern$steps[i]) == maxsteps_wend) {
        stepmax[i] <- maxsteps_wend
        }
    }
}
...
# Plot max intervals with corresponding values in the panel
p3 <- p3 + geom_point(data = subset(dt_weekpatterns, interval == maxint$interval[1]),
                      colour = "red")

p3 <- p3 + geom_text(data = subset(dt_weekpatterns, interval == maxint$interval[1]),
                      aes(x = interval,y = steps, hjust = -0.01
                          , label = paste("max = ",dailymax, "steps for interval",
                                          maxint[1,1])))
#p3
